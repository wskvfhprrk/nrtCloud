...
public class MqttMessageHandler {
    ...
    public void message(String topic, MqttMessage message) throws Exception {
        ...
        if (terminalMachine.getPassword() == null || terminalMachine.getPassword().length() == 0) {
            log.error("没有密钥，topic：{}", topic);
            return;
        }
        Boolean flag = signService.verifyData(String.valueOf(message), terminalMachine.getPassword());
        if (!flag) {
            log.error("未通过签名验证");
            mqttProviderConfig.publish(0, false, "message/error/" + topic.split("/")[1], JSON.toJSONString(AjaxResult.error("未通过签名验证")));
            return;
        }
        ...
        if (topic.split("/")[1].equals("order")) {
            ...
            if (topic.split("/")[1].equals("order")) {
                ...
                if (s.split(",")[0].equals("200")) {
                    ...
                    mqttProviderConfig.publish(0, false, "pay/" + topic.split("/")[1], JSON.toJSONString(signService.signDataToVo(dto1, terminalMachine.getPassword())));
